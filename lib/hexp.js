export function hexp(hexpEl) {
  ...
  content.forEach(hexpChild => {
    if (isString(hexpChild)) 
      el.appendChild(document.createTextNode(hexpChild));
    else if (isArray(hexpChild))
      el.appendChild(hexp(hexpChild));
  });

  return el;
}

function addEvent(el, name, value) {
  const eventName = name.slice(2).toLowerCase();
  el.addEventListener(eventName, value);
}

function addObjectAttr(el, name, value) {
  const objValue = Object.entries(value)
    .map(([k,v]) => `${k}:${v};`)
    .join('');

  el.setAttribute(name, objValue);
}
